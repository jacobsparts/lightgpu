...
pub const OPS: &[Op] = &[
    ...
    Op { name: "lg_mul", doc: "y = a * b, elementwise over n elements, out-of-place. The multiplicative twin of `lg_add`, and the op every GATED architecture needs on its own: NAFNet's SimpleGate multiplies the two halves of a channel split, MAXIM's gMLP multiplies a gate by a value. Promoted from MAXIM's mx_mul under the 'plausibly used by another family' rule in docs/MAINTAINING.md. NOT `lg_silu_mul`, which is a fused in-place gated-MLP step, and NOT a fold of a broadcast multiply, which scales every channel by one shared spatial map and is the scalar `a[0]` at hw == 1." },
    ...
    Op { name: "lg_channel_layer_norm", doc: "LayerNorm over the CHANNEL axis of an NCHW tensor: for each of the hw spatial positions, normalize the c values spaced hw apart, with w/b indexed by channel. Distinct from lg_layer_norm, which normalizes the CONTIGUOUS ne0 within each row and therefore reads the wrong axis whenever an NCHW tensor has hw > 1. One thread per spatial position, walking the c values serially with NO shared memory and no block-wide reduction: launch with grid = div_ceil(hw, blockDim) and any blockDim, which need not match the c the reduction runs over (blockDim 256 with c=32 was the old form's worst case). Distinct from lg_channel_mean, which reduces per channel and whose summation order is contractual; here the order is a plain ascending serial sum." },
